...
- 参数不越界，覆盖边界与随机分布。
- **标程输出类别分布**：若题目答案在少数离散类别上（例如 `YES`/`NO`、`0`/`1`、无解/有解），纯随机构造往往使 `gen.cpp` 产出的数据经 `std` 后**某一类占绝大多数**（两极分化），不利于覆盖两类分支。应在 `gen.cpp` 中显式控制目标类别比例（例如先以约 50% 概率决定生成「正例」或「反例」，再分别构造合法/不合法实例），或通过命令行参数（如 `balanced`、`yes100`）调节期望比例；并避免在 `gen.sh` 中长期使用会导致整组数据几乎全为同一类答案的单一模式（如全空网格、全障碍等），除非刻意测该极端。生成后可用 `std` 对大批量样例统计各类出现次数作抽查。
